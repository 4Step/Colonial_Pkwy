...
;This project uses a unique ID # to identify each pull location for the DDHVs. 
  ...
  IF (_CNT=1)
    PRINT CSV=T LIST='VOL_PULL,V_HR8_SOV,V_HR18_SOV,DY_TOT' PRINTO=1 ;include the hour link attributes to pull
  ENDIF

    IF (li.1.VOL_PULL>0)
    PRINT CSV=T LIST=VOL_PULL,V_HR8_SOV,V_HR18_SOV,DY_TOT,PRINTO=1
  ENDIF
